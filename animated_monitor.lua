...
local bootstrapPath = fs.combine(baseDir, "lib/animator/bootstrap.lua")

if not fs.exists(bootstrapPath) then
    error("Missing lib/animator/bootstrap.lua. Run install.lua first.")
end

local bootstrap = assert(loadfile(bootstrapPath))()
bootstrap.run("animator.app", ...)
